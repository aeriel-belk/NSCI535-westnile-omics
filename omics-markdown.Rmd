...
ggplot() +
  ...
  geom_sf(data = ca_wnv_sf, aes(color = season,
                                shape = species))
  scale_fill_viridis_c(name = "Season")
...
length(cons_seq[[1]]) #length of first sequence in the multi FASTA file

#number of occurances of each character
alphabetFrequency(cons_seq, baseOnly=TRUE, collapse=TRUE) #total 
#      A       C       G       T   other  ##'other' is N or unknown/any nt character 
#1847658 1495638 1935723 1460705  342960 


freqs <- alphabetFrequency(DNAStringSet(cons_seq[!is.na(cons_seq)]), baseOnly=TRUE) # number of bases by sequence 
head(freqs)
# serious amount of variation in number of Ns in each sequence. Sequencing error with poor quality samples?
...
writeXStringSet(wnv_adjusted,file="fasta/wnv_adjusted.fas",format="fasta") ## write aligned sequence DNAstringset to fasta

#use ShortRead package to trim off the ends from base # 31-10963 of the data where there are a lot of missing values.  
wnv_clean <- DNAStringSet(substr('fasta/wnv_adjusted',31,10963))
wnv_clean 
BrowseSeqs(wnv_clean, highlight=0) #visualize in browser 

writeXStringSet(wnv_clean,file="fasta/wnv_clean.fas",format="fasta")
...
#  618 haplotypes
#  12712 links
# link lengths between 1 and 21 steps

ind.hap <-with(stack(setNames(attr(wnv_hap, "index"), rownames(wnv_hap))),
table(hap=ind, pop=rownames(fasta.read)[values])
)

#Number of haplotypes: 618 Sequence length: 10933....Poor quality data causing such variation?  
#looking at it, there are multiple reads that are considerably shorter, or have a singificant amount of Ns you can see 
#no clean easy way to clean FASTA files in R. Use python script instead 
#use python script "sequence_cleaner.py" from Biopython (genivaldo.gueiros@gmail.com) and select reads that are at least 10317 (based on eye balling where the large stretches of gaps were), and only allow sequences with 10% N content
#output clear_wnv_clean.fas
```
