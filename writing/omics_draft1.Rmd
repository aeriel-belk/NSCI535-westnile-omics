...
```
  ...
  To answer this question,a PCA analysis wa performed on the Sacramento, Kern, and San Diego samples by first converting our filtered fasta files to a geid object using the adegenet and dartR packages, so that only the veritable regions of the sequences were conserved. This polymorphic data set was then run through a PCA and plotted using ggplot and ggfortify. 
```{r, eval=FALSE, include=FALSE}
